...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        ################################################################
        distance = math.sqrt((point.x-self.circle.center.x)**2+(
                point.y-self.circle.center.y)**2)
        return distance
    ...
    def swallow(self, other_circle_changer):
        ...
        Type hints:
            ...
            :rtype CircleChanger
        """
        center = rg.Point((self.circle.center.x+other_circle_changer.circle
                          .center.x)/2,
                          (self.circle.center.y+other_circle_changer.circle
                          .center.y)/2)
        radius = self.get_distance_from(other_circle_changer.circle.center)/2
        fill_color = 'red'
        colors = self.colors+other_circle_changer.colors
        return CircleChanger(center.x,center.y,radius,fill_color,colors)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.circle.fill_color = self.colors[(self.count+1)%len(self.colors)-1]
